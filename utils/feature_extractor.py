...
Specifically designed for Intel's pre-silicon validation log format.
"""
import numpy as np
import pandas as pd
...
class PtrackerFeatureExtractor:
    def __init__(self, logger=None, reference_model=None):
        """
        Initialize the Feature Extractor for ptracker logs

        Parameters:
        logger (logging.Logger): Logger instance
        reference_model: Optional reference model for anomaly detection
        """
    ...
    def extract_features(self, tests, feature_types=None):
        ...
        for test_id, test_data in tests.items():
            ...
            # Perform anomaly detection if reference model is provided
            if self.reference_model and hasattr(self.reference_model, 'detect_anomalies'):
                anomalies = self.reference_model.detect_anomalies(test_data)
                test_data['anomalies'] = anomalies
    ...
    def save_models(self, output_dir):
        """
        Save feature extraction models to disk
    ...
    def load_models(self, model_dir):
        """
        Load feature extraction models from disk
